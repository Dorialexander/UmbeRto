...
um_model <- function(log_data, knots = 3, min_observations = 7, min_anomaly_ratio = 5) {
  #We create a model for each publication in log_data.
  anomaly_prediction_models = log_data %>%
    group_by(document) %>%
  ...
  anomaly_prediction <- anomaly_prediction %>%
    inner_join(log_data %>% select(document, time_variable, time_sequency), by=c("document", "time_sequency")) %>%
    select(-fit, -lwr, -upr)
