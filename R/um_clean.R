...
#' @param time_sequence The exact name of the date of records column.
#' @param count_log The exact name of the aggregated logs column.
#' @examples
#'  um_clean(log_data, document, recorded_date, logs)
um_clean <- function(log_data, document, time_variable, count_log) {
  ...
  time_variable_col = quo_name(enquo(time_variable))
  count_log_col = quo_name(enquo(count_log))
